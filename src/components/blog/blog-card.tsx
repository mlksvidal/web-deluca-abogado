  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 px-6 py-5">
        ...
        {/* Título */}
        <h2 className="font-serif text-xl font-semibold text-[var(--color-marino)] leading-snug mb-3 line-clamp-2">
          <Link
            href={`/blog/${post.slug}`}
            className={cn(
              "hover:text-[var(--color-marino-hover)] transition-colors duration-150",
              "focus-visible:outline-2 focus-visible:outline-[var(--color-dorado)] focus-visible:outline-offset-2 focus-visible:rounded-[2px]"
            )}
          >
            {post.title}
          </Link>
        ...
        {/* Excerpt */}
        <p className="font-body text-sm text-[var(--color-carbon-soft)] leading-relaxed flex-1 mb-4 line-clamp-3">
          {post.excerpt}
        </p>

        {/* Meta: fecha + autor */}
        <div className="flex items-center justify-between gap-2 pt-4 border-t border-[var(--color-border-default)]">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            {publishedDate && (
              <span className="flex items-center gap-1.5 font-ui text-xs text-[var(--color-text-tertiary)] whitespace-nowrap">
                <Calendar size={12} aria-hidden="true" />
                <time dateTime={post.publishedAt?.toISOString()}>{publishedDate}</time>
              </span>
            )}
            <span className="flex items-center gap-1.5 font-ui text-xs text-[var(--color-text-tertiary)] min-w-0">
          ...
          <Link
            ...
            className={cn(
              ...
              "font-ui text-xs font-semibold text-[var(--color-dorado-deep)]",
